  ...
  return (
    <div className="page-content">
      ...
      <div className="controls-bar">
        ...
        <div style={{ flex: 1 }} />
        <button className="btn btn-primary" onClick={() => openModal()}>+ Duiker toevoegen</button>
      </div>
      <div className="table-wrap"><div className="tbl-scroll">
        <table>
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={10}><div className="empty-state"><strong>Geen duikers gevonden</strong></div></td></tr>
            ) : rows.map(d => (
              <tr key={d.id} style={{ cursor: 'pointer' }} onClick={() => openModal(d.id)}>
                <td style={{ fontWeight: 600 }}>{d.duiker_nr}</td>
                <td>{d.type || '—'}</td><td>{d.materiaal || '—'}</td>
                <td style={{ maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis' }}>{d.locatie || '—'}</td>
                <td>{d.diameter_mm ?? '—'}</td><td>{d.lengte_m ?? '—'}</td><td>{d.aannemer || '—'}</td>
